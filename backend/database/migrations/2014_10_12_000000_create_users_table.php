...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->boolean('activated')->default(0)->index();// 是否激活
            $table->rememberToken()->nullable()->index();
            $table->dateTime('created_at')->index(); // 注册时间
            $table->dateTime('deleted_at')->nullable()->index();// 软删除必备
            $table->integer('level')->default(0)->index(); // 用户等级
            $table->integer('title_id')->default(0)->index(); // 头衔ID
            $table->string('role', 10)->nullable()->index(); // 站内身份
            $table->integer('quiz_level')->default(0)->index(); // 用户答题等级
