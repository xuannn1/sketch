...
class StoreReward extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'reward_value' => 'required|numeric|between:-1,10',
        ];
    }
    ...
    public function validateDateAndAttribite($rewarded_model,$reward_data){
        ...
        ->where('reward_type',$reward_data['reward_type'])
        ->get()->isEmpty();

    }

    public function validateBalance($reward_data,$user){
        return $user->info()->value($reward_data['reward_type']) >= $reward_data['reward_value'];

    }

    public function generateReward($rewarded_model){
        if(!in_array($this->attribute, $this->rewards)){abort(422,'不存在这种投票');}
