...
<div class="selected">
    ...
    <a class="btn btn-info btn-md sosad-button-control" href="{{ route('thread.show',
    array_merge(['thread'=>$thread->id], request()->only('withType', 'withFolded', 'userOnly', 'withReplyTo', 'ordered'))) }}" role="button">{{ $selector['withComponent'][request()->withComponent] }}<span class="glyphicon glyphicon-remove"></span></a>
    @endif
    @if(request()->withFolded&&array_key_exists(request()->withFolded,$selector['withFolded']))
    <a class="btn btn-info btn-md sosad-button-control" href="{{ route('thread.show',
    array_merge(['thread'=>$thread->id], request()->only('withType', 'withComponent', 'userOnly', 'withReplyTo', 'ordered'))) }}" role="button">{{ $selector['withFolded'][request()->withFolded] }}<span class="glyphicon glyphicon-remove"></span></a>
    @endif
    @if(request()->userOnly)
    <a class="btn btn-info btn-md sosad-button-control" href="{{ route('thread.show',
