...
@else
    ...
    <div class="panel-group h4">
        <form id="replyToThread" action="{{ route('post.store', $thread) }}" method="POST">
            ...
            <h6 style="color:#d66666">(问题箱不意味着可以提“任意问题”，提问前需先看首楼规则，不可以提作者明确说不愿回答的问题。严禁提“等级”、“签到”、“边限”、“提问”、“看不见”等各种和作者及作品无关的网站使用问题骚扰（使用疑难请自行“搜索”关键词，或仔细阅读《帮助》）。)</h6>
            @endif
            @if(!$thread->channel()->allow_anonymous)
            <h6 style="color:#d66666">（投诉仲裁至关严肃，禁止批马禁止修改留言。未按版规举证的留言一律折叠，视情况禁言禁止登陆）</h6>
            @endif
            <div class="checkbox">
