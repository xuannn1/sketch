    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('admin.statusmanagement',$status->id)}}" method="POST">
                ...
                @if($status->is_public)
                <div class="radio">
                    <label><input type="radio" name="controlstatus" value="61">动态转私密</label>
                ...
                <div class="radio">
                    <label><input type="radio" name="controlstatus" value="66">删+清+封（动态删除，积分等级清零，发言人禁止登陆1天）</label>
                    <h6 class="grayout">违反商业性规定/恋童</h6>
                </div>

                <div class="radio">
                    <label><input type="radio" name="controlstatus" value="36">删+封（动态删除，发言人永久禁止登陆）</label>
                    <h6 class="grayout">恶意广告</h6>
                </div>
                @else
                <div class="radio">
                    <label><input type="radio" name="controlstatus" value="62">取消私密</label>
                </div>
                @endif

                <div class="radio admin-symbol pull-right">
                    <label><input type="radio" name="controlstatus" value="17">动态删除</label>
                </div>
